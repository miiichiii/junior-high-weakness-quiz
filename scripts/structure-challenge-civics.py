#!/usr/bin/env python3
"""Structure the private Challenge civics scan into a source-grounded library."""
...
UNITS = [
    ("civ-22", "わたしたちの生活と現代社会", 44, 45),
    ("civ-23", "日本国憲法と基本的人権", 46, 47),
    ("civ-24", "現代の民主政治・国会", 48, 49),
    ("civ-25", "内閣・裁判所・三権分立", 50, 51),
    ("civ-26", "地方自治", 52, 53),
    ("civ-27", "わたしたちのくらしと経済", 54, 55),
    ("civ-28", "国民生活と福祉", 56, 57),
    ("civ-29", "国際社会と世界平和", 58, 59),
]
...
def find_source_pdf(source_dir: Path) -> Path:
    ...
    if len(pdfs) != 1:
        raise ValueError(f"Expected exactly one civics PDF, found {len(pdfs)}: {[p.name for p in pdfs]}")
    if normalized_name(pdfs[0]) != "22-29公民":
        raise ValueError(f"Expected 22-29公民.pdf, found {pdfs[0].name}")
    return pdfs[0]
...
def candidate_lines(text: str, limit: int = 80) -> list[str]:
    skip = re.compile(r"^(\d{1,2}|公民|これが重要|ポイントチェック)$")
    seen: set[str] = set()
    result: list[str] = []
    for raw in text.splitlines():
        line = raw.strip(" -|\t")
        compact = re.sub(r"\s+", "", line)
        if len(compact) < 4 or len(compact) > 120 or skip.match(compact) or compact in seen:
            continue
        seen.add(compact)
        result.append(line)
        if len(result) >= limit:
...
def figure_candidates(text: str) -> list[str]:
    marker = re.compile(r"(図|表|グラフ|資料|割合|しくみ|組織|推移|構成|制度)")
    return [line for line in candidate_lines(text, 160) if marker.search(line)][:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Structure Challenge社会・公民 scan pages into private AI reference data.")
    ...
    if pdf_page_count(pdf) != len(UNITS):
        raise ValueError("Expected 8 PDF pages / spreads")
    ...
    with tempfile.TemporaryDirectory(prefix="challenge-civics-") as temp_name:
        rendered = render_pdf(pdf, Path(temp_name))
        if len(rendered) != len(UNITS):
            raise ValueError(f"Render count mismatch: expected 8, found {len(rendered)}")
        for pdf_index, ((unit_id, title, first_page, last_page), rendered_path) in enumerate(zip(UNITS, rendered), start=1):
            ...
            with Image.open(rendered_path) as spread:
                ...
                crops = [
                    spread.crop((0, 0, midpoint, spread.height)),
                    spread.crop((midpoint, 0, spread.width, spread.height)),
                ]
            ...
            for printed_page, side, crop in zip((first_page, last_page), ("left", "right"), crops):
                ...
                page_md = "\n".join([
                    ...
                    f"source_pdf_page: {pdf_index}", f"page_image: ../{image_rel}",
            ...
            unit_number = int(unit_id.split("-")[1])
            unit_md = "\n".join([
            ...
            for index, caption in enumerate(figure_candidates(combined_text), start=1):
                figure_rows.append({
                    "figure_id": f"{unit_id}-candidate-{index:02d}",
                    "unit_id": unit_id,
                    "caption_ocr": caption,
                    "printed_pages": [first_page, last_page],
                    "spread_image": f"page_images/spreads/{spread_name}",
    ...
    sections_index = ["# 公民単元索引", "", "| 単元 | 見出し | 印刷ページ | Markdown |", "|---|---|---:|---|"]
    sections_index.extend(
    ...
    write_json(output / "ai_manifest.json", {
        "name": "Challenge社会・公民 構造化データ",
        "entrypoint_for_ai": "AI_README.md",
        "source_directory": str(source_dir),
        "privacy": "private; do not copy PDFs, OCR text, or page images into the public app",
        "counts": {"source_pdfs": 1, "spreads": 8, "printed_pages": 16, "units": 8},
        "indexes": {
            "page_map": "page_map.csv", "spread_map": "spread_map.csv", "sections": "sections_index.md",
            "figures": "figures_index.md", "pages_jsonl": "textbook_pages.jsonl",
            "validation": "reports/validation_report.md",
        },
    })
    (output / "AI_README.md").write_text("""# AI向け参照ガイド: Challenge社会・公民
...
- 問題は根拠事項から新しく作り、教材の設問・文章・図表を転載しない。
""", encoding="utf-8")
    ...
    counts_ok = (
        len(spread_rows) == 8 and len(page_rows) == 16
        and len(list(dirs["spreads"].glob("*.jpg"))) == 8
        and len(list(dirs["pages"].glob("*.jpg"))) == 16
        and len(list(dirs["markdown"].glob("*.md"))) == 16
        and len(list(dirs["units"].glob("*.md"))) == 8
        and len(list(dirs["facts"].glob("*.json"))) == 8
    )
    verdict = "WARNING" if counts_ok and not ocr_empty else "FAIL"
    validation = "\n".join([
        "# 構造化検証レポート", "", "## 総合判定", "", verdict, "", "## 件数", "",
        "- 元PDF: 1 / 1", f"- 見開き: {len(spread_rows)} / 8", f"- 印刷ページ: {len(page_rows)} / 16",
        f"- 単元Markdown: {len(section_rows)} / 8", f"- ページ画像: {len(list(dirs['pages'].glob('*.jpg')))} / 16",
        f"- 件数判定: {'PASS' if counts_ok else 'FAIL'}", "", "## OCR", "", f"- 空ページ: {ocr_empty or 'なし'}",
        f"- 80文字未満: {ocr_short or 'なし'}", "- 判定: WARNING。OCRは検索用で、作問時は原画像を照合する。", "",
        "## ページ対応", "", "- civ-22 p.44-45 から civ-29 p.58-59 まで連続している。",
        "- PDF内ページと印刷ページは page_map.csv で別々に保持している。", "",
    ])
    (dirs["reports"] / "validation_report.md").write_text(validation, encoding="utf-8")
    print(json.dumps({"output": str(output), "verdict": verdict, "spreads": 8, "printed_pages": 16, "ocr_empty": ocr_empty}, ensure_ascii=False))
