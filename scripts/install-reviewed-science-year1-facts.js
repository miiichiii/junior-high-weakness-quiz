...
if (libraryIndex < 0 || !process.argv[libraryIndex + 1]) throw new Error("Usage: install-reviewed-science-year1-facts.js --library <private-library>");
...
vm.runInContext(fs.readFileSync(path.join(ROOT, "data/challenge-science-year1.js"), "utf8"), context);
const questions = window.QUIZ_QUESTIONS.filter((question) => question.packId === "challenge-science-year1");
if (questions.length !== 192) throw new Error(`Expected 192 questions, found ${questions.length}`);
...
const report = [
  "# 理科1年 根拠照合・入試対応レビュー", "", "## 判定", "", "PASS", "",
  "- 対象: sci1-01〜sci1-08、192問。", "- OCRは検索補助に限定し、8見開きの原画像を正本として照合した。",
  ...
  "- 密度、濃度、溶解度、ばね、音、地震波の計算は式・単位・往復条件まで再計算した。", "",
  "## 限界", "", "理科1年範囲の復習コースであり、合格には2・3年範囲と時間制限付きの年度別過去問演習も必要。", ""
].join("\n");
